...
if [[ $STAGE -le 0 ]]; then
    ...
    docker build -t jaja123456/jellyfin-runtime:v1 -f Dockerfile.runtime deployment || exit 1
    #docker push jjm2473/jellyfin-runtime:v1
fi

if [[ $STAGE -le 1 ]]; then
    # build this jellyfin server
    docker build -t jaja123456/jellyfin-server-build:$VERSION -f deployment/Dockerfile.docker.arm64 . || exit 1
    docker build -t jaja123456/jellyfin-server:$VERSION --build-arg VERSION=$VERSION -f Dockerfile.server deployment || exit 1
...
if [[ $STAGE -le 3 ]]; then
    ...
    # create common image
    docker build -t jaja123456/jellyfin-base:$VERSION --build-arg VERSION=$VERSION -f Dockerfile.base common || exit 1
    ## push local repo, ignore error
    # docker tag jaja123456/jellyfin-base:$VERSION localhost:5000/jaja123456/jellyfin-base:$VERSION
    # docker push localhost:5000/jaja123456/jellyfin-base:$VERSION 2>/dev/null
...
if [[ $STAGE -le 4 ]]; then
    # build rtd1296 image
    #[ -f realtek/Dockerfile.sample ] || ( mkdir -p realtek && curl -L https://github.com/jjm2473/rtd1296_prebuilt_target/releases/download/v1.0/jellyfin-rtk-ffmpeg_4.0.3.tar | tar -z -x -C realtek ) || exit 1

    #docker build --pull -t jjm2473/jellyfin-rtk:4.9-latest --build-arg VERSION=$VERSION -f Dockerfile.rtk realtek || exit 1

    # build rockchip image
    [ -f rockchip/Dockerfile.sample ] || ( mkdir -p rockchip && curl -L https://github.com/jjm2473/ffmpeg-rk/releases/download/v1.0/jellyfin-rkmpp-ffmpeg_5.1-2.git.tar | tar -z -x -C rockchip ) || exit 1

    docker build --pull=false -t jaja123456/jellyfin-mpp:$VERSION --build-arg VERSION=$VERSION -f Dockerfile.rk rockchip || exit 1
fi
